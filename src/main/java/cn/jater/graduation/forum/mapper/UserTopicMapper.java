...
public interface UserTopicMapper extends UserTopicService {
    ...
    @Select({"select t.*, u.is_like from topic t " +
            ...
            "where t.classify = #{name} order by t.create_time desc limit #{start}, #{pageSize}"})
    @Results({
    ...
    @Override
    @Select({"select * from topic where classify = #{name} order by create_time desc limit #{start}, #{pageSize}"})
    @Results({
            @Result(property = "_id", column = "_id"),
            @Result(property = "cover", column = "_id",
                    many = @Many(select = "cn.jater.graduation.forum.mapper.TopicMapper.findCoverByTopicId",
    ...
    @Override
    @Select({"select * from topic order by browse_count desc limit #{start}, #{pageSize}"})
    @Results({
            @Result(property = "_id", column = "_id"),
            @Result(property = "cover", column = "_id",
                    many = @Many(select = "cn.jater.graduation.forum.mapper.TopicMapper.findCoverByTopicId",
